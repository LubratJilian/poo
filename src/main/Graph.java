 ...
 *
 * Notice: for all methods taking main.Vertex as parameter, calling the method
 * with a vertex which doesn't belong to the graph will raise a
 * main.BadVertexException exception
 */
public interface Graph {
	 ...
	 * Notice: for main.DiGraph, degree(v) = inDegree(v) + outDegree(v)
	 */
	 int degree(Vertex u);
	
	/**
	 * Returns the in-degree of vertex 'u'
	 * Notice: for main.UnDiGraph, inDegree(v) = degree(v)
	 */	
	 int inDegree(Vertex u);
	
	/**
	 * Returns the out-degree of vertex 'u'
	 * Notice: for main.UnDiGraph, outDegree(v) = degree(v)
	 */		
